...
def _find_template():
    ...
    for _ in range(8):
        cand = os.path.join(d, "题目", "result1.xlsx")
        if os.path.exists(cand):
            return cand
        d = os.path.dirname(d)
    raise FileNotFoundError("未找到 题目/result1.xlsx 官方模板")
...
def _write_excel(outdir, X, Y, Vv):
    """填充官方模板：只写数据单元格，第1行/第1列标签零改动（准备库03）。

    位置 sheet：node i -> 行 (2+2i) 为 x, (3+2i) 为 y；数据列 2..302 对应 0..300 s。
    ...
    for i in range(N_NODE):
        for j in range(nt):
            ...
            ws2.cell(2 + i, 2 + j).value = round(float(Vv[i, j]), 6)

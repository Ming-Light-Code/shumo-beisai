...
% 检查Optimization Toolbox是否可用
if ~license('test','Optimization_Toolbox')
    error('本模型依赖Optimization Toolbox (intlinprog)，请安装后重试。');
end
if nargin<2 || isempty(randomSeed), randomSeed=2026; end

% 资源充分性预检查: 估算直航B→E最低资源需求 vs 可用资源总量
minStepsToE = sum(abs(p.B - p.E));
avgConsumptionPerStep = p.pNormal*sum(p.normalMove) + p.pStorm*sum(p.stormMove);
minResourcesNeeded = minStepsToE * avgConsumptionPerStep;
totalInitResources = 100 + 150 + 100;  % 燃油+淡水+食物
maxPurchaseResources = floor(750 / min(p.price));
totalAvailable = totalInitResources + maxPurchaseResources;
fprintf('资源预算: 直航B->E至少需要约%.0f单位, 含资金可支撑约%.0f单位\n', ...
    minResourcesNeeded, totalAvailable);
if minResourcesNeeded > totalAvailable
    warning('资源可能不足以完成最基本的B->E直航!');
end
...
p.lookahead=20; p.maxSolveTime=30; p.relativeGap=0.005;
p.minTravelToEnd=sum(abs(p.B-p.E));
p.nScenarios=25;  % K weather scenarios for stochastic MILP (up from 5)
...
% 基线对比: 计算理论上界和朴素下界
% 理论上界: 假设全部时间在最佳作业点工作 (W3: 28/天, 最多连续3天)
maxWorkDaysTotal = 0; for w=1:3, maxWorkDaysTotal = maxWorkDaysTotal + p.maxWork(w); end
ubZ = 200 + maxWorkDaysTotal * max(p.reward);  % 初始Z + 全作业收益
ubZ = min(ubZ, 200 + 90*max(p.reward));  % 不可能超过90天全作业
% 朴素下界: 直航B->E，不作业不补给
lbMinSteps = sum(abs(p.B - p.E));
totalConsumption = lbMinSteps * (p.pNormal*sum(p.normalMove) + p.pStorm*sum(p.stormMove));
lbFeasible = (totalConsumption <= 100+150+100);  % 初始资源是否足够直航
    feasStr = '不足'; if lbFeasible, feasStr = '可行'; end
    fprintf('理论上界: Z <= %d,  朴素下界: 直航Z=200 (资源%%s)\n', ubZ, feasStr);
if q.feasible
    utilization = (q.teamZ - 200) / max(1, ubZ - 200);
    fprintf('MPC方案Z=%d, 利用率=%.1f%%\n', q.teamZ, utilization*100);
end
% 输出
...
% 资源索引约定：struct字段名与题目符号的对应关系：
%   Water字段 → 存储燃油(O)=100 (索引1, price=2, move消耗=2)
%   Fuel字段  → 存储淡水(H)=150 (索引2, price=1, move消耗=3)
%   Food字段  → 存储食物(F)=100 (索引3, price=2, move消耗=2)
% MILP内部和蒙特卡洛验证均使用相同索引，逻辑一致。
...
nScen=min(p.nScenarios,40);  % cap raised from 5 to 40 for better tail coverage
...
% Simplified chance constraint: direct quantile without Bonferroni decomposition
stageAlpha=p.arrivalProbability;  % direct use of target probability
...
for w=1:nW
% Graduated evacuation: per-point work day budget based on distance to E
% A work point w is usable only if: work_days + dist(w,E) + buffer <= remaining
safetyBuffer=5;  % day buffer for storm-related delays
for w=1:nW
    distWtoE=sum(abs(p.W(w,:)-p.E));
    maxPossibleWork=fullRemaining - distWtoE - safetyBuffer;
    if maxPossibleWork<=0
        ub(workIdx(w,:))=0;
    else
        % Graduated cap based on time budget, not exceeding max consecutive work
        cappedMaxWork=min(p.maxWork(w), maxPossibleWork);
        for h=(cappedMaxWork+1):H
            ub(workIdx(w,h))=0;
        end
    end
end
...
if exist('optimoptions','file')
    ...
    warning('optimoptions不可用，使用optimset默认选项。');
end
% Terminal Z potential: estimate remaining work value beyond lookahead window
% Terminal Z potential: resource-aware estimate of feasible work beyond lookahead
% Expected work-day consumption and available resources determine discount factor
workCostPerDay = p.pNormal*sum(p.normalWork) + p.pStorm*sum(p.stormWork);
avgMoveCostPerDay = p.pNormal*sum(p.normalMove) + p.pStorm*sum(p.stormMove);
% Discount factor: resource slack / total resources, bounded [0.05, 0.30]
maxPuchasable = p.capacity;  % rough upper bound on purchasable resources
totalResourceCapacity = maxPuchasable + initial.Water + initial.Fuel + initial.Food;
workDiscount = min(0.30, max(0.05, 0.12 + 0.08*(fullRemaining/p.T)));
terminalDist=sum(abs(nodes-p.E),2);
remainingAfterWindow=fullRemaining-H;
terminalZPotential=max(0,remainingAfterWindow-terminalDist)*max(p.reward)*workDiscount;

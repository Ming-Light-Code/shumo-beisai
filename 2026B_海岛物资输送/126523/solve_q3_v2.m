function solve_q3()
%% TASK3: Two-Stage Stochastic Optimization for 30x30 grid, 90 days
...
d=zeros(7); for i=1:7, for j=1:7, d(i,j)=abs(all_xy(i,1)-all_xy(j,1))+abs(all_xy(i,2)-all_xy(j,2)); end; end
...
%% ===== PHASE 2: MILP on Top Candidates =====
...
fprintf("\n=== Phase 4: Daily Rolling Horizon ===\n");
wx=rand(1,MD)>0.8;[S,Z,M]=phase4_daily(best_sk,wx,all_xy,CMn,CWn,CSn,CMs,CWs,CSs,WY,WM,LD,MD,CMe,CWe,CSe,VM,VW,VS,IO,IH,IF,IM,IZ);
...
% Forward sim with safety buffer
by=zeros(ns,3); O=IO; H=IH; F=IF; M=IM;
for t=1:T
  O=O-cO(t); H=H-cH(t); F=F-cF(t);
  if O<0||H<0||F<0, ok=false; Z=0; M=0; return; end
  if t<=length(iS)&&iS(t)
    % Compute remaining needs with safety
    no=0; nh=0; nf=0; remT=0;
    for tt=t+1:T
      if tt<=length(iS)&&iS(tt), break; end
      no=no+cO(tt); nh=nh+cH(tt); nf=nf+cF(tt); remT=remT+1;
    end
...
%% ======================================================================
%% Stochastic lookahead decision
%% ======================================================================
function act=stoch_decision(S,Tr,w,sk,all_xy,POI,CMn,CWn,CSn,CMs,CWs,CSs,WY,WM,LD,CMe,CWe,CSe,VM,VW,VS,Zf,K,MD)
% Find current POI index
cpi=0; for i=1:7, if isequal(S.pos,all_xy(i,:)), cpi=i; break; end; end
% If at E, no decision needed
if cpi==2, act=struct("type","arrive"); return; end

% Determine candidates: next POI to head to
if cpi==0
  % Between POIs: continue toward next target in skeleton
  act=struct("type","move","t",S.pos,"buyO",0,"buyH",0,"buyF",0,"cost",0);
  dx=sign(all_xy(2,1)-S.pos(1)); dy=sign(all_xy(2,2)-S.pos(2));
  if dx~=0, act.t=S.pos+[dx,0]; elseif dy~=0, act.t=S.pos+[0,dy]; end
  return;
end

% At a named POI: evaluate next destinations
candidates=POI(POI~=cpi); % all other POIs
% Also consider going directly to E
if cpi~=2, candidates=[candidates,2]; end

best_EZ=-inf; best_act=struct("type","moor","t",S.pos,"buyO",0,"buyH",0,"buyF",0,"cost",0);

for cand=candidates
  % For this candidate, construct a sub-skeleton: [cpi, cand, ...]
  % The rest follows the base skeleton order
  % For simplicity, create a simplified plan
  
  % Compute distance to candidate
  dist_cand=abs(S.pos(1)-all_xy(cand,1))+abs(S.pos(2)-all_xy(cand,2));
  if dist_cand+5>Tr, continue; end % too far
  
  Zsum=0; succ=0;
  for ki=1:K
    % Generate weather for remaining days
    Tr_est=Tr;
    wx=rand(1,Tr_est)>0.8;
    
    % Simulate: first move to candidate, then just go to E
    temps=S; ok=true;
    
    % Move to candidate
    cur=S.pos; target=all_xy(cand,:);
    while ~isequal(cur,target) && temps.day<MD
      temps.day=temps.day+1;
      if temps.day>length(wx), break; end
      nxt=cur; if nxt(1)~=target(1), nxt(1)=nxt(1)+sign(target(1)-nxt(1)); else nxt(2)=nxt(2)+sign(target(2)-nxt(2)); end
      if wx(temps.day)==0, CMt=CMn; else CMt=CMs; end
      temps.O=temps.O-CMt(1); temps.H=temps.H-CMt(2); temps.F=temps.F-CMt(3);
      if temps.O<0||temps.H<0||temps.F<0, ok=false; break; end
      cur=nxt;
    end
    if ~ok, continue; end
    
    % Work at candidate if it is a work POI
    if cand>=3&&cand<=5 && temps.cw<WM(cand-2)
      wh=cand-2;
      for wd=1:min(WM(wh),Tr)
        temps.day=temps.day+1; if temps.day>MD||temps.day>length(wx), break; end
        if wx(temps.day)==0, CWt=CWn; else CWt=CWs; end
        temps.O=temps.O-CWt(1); temps.H=temps.H-CWt(2); temps.F=temps.F-CWt(3);
        if temps.O<0||temps.H<0||temps.F<0, ok=false; break; end
        temps.Z=temps.Z+WY(wh);
      end
      if ~ok, continue; end
    end
    
    % Supply if candidate is supply POI
    if cand==6||cand==7
      rem_days=abs(temps.pos(1)-all_xy(2,1))+abs(temps.pos(2)-all_xy(2,2));
      eO=rem_days*CMe(1); eH=rem_days*CMe(2); eF=rem_days*CMe(3);
      bufO=Zf*sqrt(rem_days*VM(1)); bufH=Zf*sqrt(rem_days*VW(1)); bufF=Zf*sqrt(rem_days*VS(1));
      buyO=max(0,eO+0.5*bufO-temps.O); buyH=max(0,eH+0.5*bufH-temps.H); buyF=max(0,eF+0.5*bufF-temps.F);
      free=LD-(temps.O+temps.H+temps.F); tb=buyO+buyH+buyF;
      if tb>free, sc=free/max(tb,1); buyO=floor(buyO*sc); buyH=floor(buyH*sc); buyF=floor(buyF*sc); end
      cost=buyO*2+buyH+buyF*2;
      if cost<=temps.M, temps.O=temps.O+buyO; temps.H=temps.H+buyH; temps.F=temps.F+buyF; temps.M=temps.M-cost; end
    end
    
    % Move to E
    targetE=all_xy(2,:);
    while ~isequal(temps.pos,targetE) && temps.day<MD
      temps.day=temps.day+1; if temps.day>length(wx), break; end
      nxt=temps.pos; if nxt(1)~=targetE(1), nxt(1)=nxt(1)+sign(targetE(1)-nxt(1)); else nxt(2)=nxt(2)+sign(targetE(2)-nxt(2)); end
      if wx(temps.day)==0, CMt=CMn; else CMt=CMs; end
      temps.O=temps.O-CMt(1); temps.H=temps.H-CMt(2); temps.F=temps.F-CMt(3);
      if temps.O<0||temps.H<0||temps.F<0, ok=false; break; end
      temps.pos=nxt;
  ...
  if EZ>best_EZ
    ...
    act=struct("type","move","t",S.pos,"buyO",0,"buyH",0,"buyF",0,"cost",0);
    target=all_xy(cand,:);
    dx=sign(target(1)-S.pos(1)); dy=sign(target(2)-S.pos(2));
    if dx~=0, act.t=S.pos+[dx,0]; elseif dy~=0, act.t=S.pos+[0,dy]; end
    if cand==cpi, act.type="moor"; end
    % If at supply POI and heading somewhere else, consider supplying first
    if (cpi==6||cpi==7) && cand~=cpi
      act.type="supply";
    end
  end
end
...
%% ===== Execute skeleton with adaptive supply =====

function milp_task1_v6()
% ================================================================
% MILP Solution 3 v6 - Stop-Work Strategy
%
% New: "Stop-then-work" -- insert 1-day stop breaks between work
% blocks to reset the consecutive-work counter. Each work visit can
% now have: work(block1) -> stop(1day) -> work(block2)
%
% Variables per work visit: w1_j <= WM, b_j in {0,1}, w2_j <= b_j*WM
% ================================================================
...
for si=1:n_seqs
    ...
    if n_work==0
        [gfeas,gZ,gM,gBuy]=greedy_quick_sw(m,travel,work_idx,work_which,supp_idx,...
    ...
    if gfeas_max&&gZ_max>=Z_upper
        ...
        [feas2,~,M2,w1_o2,b_o2,w2_o2,buy_o2]=milp_stage2_sw(m,travel,work_idx,...
        ...
        [feas,Z,M,w1_o,b_o,w2_o,buy_o]=milp_full_sw(m,travel,work_idx,...
            work_which,supp_idx,n_work,n_supply,total_travel,WY,WM,CM,CW,CS,...
...
fprintf('\n========================================\n');
fprintf('  MILP v6 (Stop-Work) - Optimal Result\n');
fprintf('========================================\n');
fprintf('Total time:       %.2f s\n',elapsed_total);
fprintf('Stop-Work:        ENABLED\n');
...
% =================================================================
function [feasible,Z_opt,M_opt,w1_opt,b_opt,w2_opt,buy_opt]=milp_full_sw(...
    m,travel,work_idx,work_which,supp_idx,n_work,n_supply,total_travel,...
...
% =================================================================
function [feasible,Z_opt,M_opt,w1_opt,b_opt,w2_opt,buy_opt]=milp_stage2_sw(...
    m,travel,work_idx,work_which,supp_idx,n_work,n_supply,total_travel,...
...
lb(off_M+1)=INIT_M;ub(off_M+1)=INIT_M;
for j=1:n_work
    ub(off_b+j)=1;ub(off_w1+j)=WM(work_which(j));ub(off_w2+j)=WM(work_which(j));
end
opts=optimoptions('intlinprog','Display','off');
...
for j=1:n_work
    ...
    % b_j <= 1 handled by ub(off_b+j)=1, no inequality needed here
end
for i=1:(m+1)
    if supp_idx(i)>0
        d=travel(i);

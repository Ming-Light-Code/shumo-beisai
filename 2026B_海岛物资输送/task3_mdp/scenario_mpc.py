...
def monte_carlo_validation(V, n=300, seed=42):
    print('\nMC validation (%d runs)...' % n)
    np.random.seed(seed)
    succ = 0; zs, ms, ars = [], [], []
    for i in range(n):
        w = ['storm' if np.random.random() < p.p_storm else 'normal' for _ in range(p.T)]
        r = run_simulation(w, V, verbose=False)
        if r['feasible']:
            succ += 1; zs.append(r['final_Z']); ms.append(r['final_M']); ars.append(r['arrival_day'])
        if (i + 1) % 100 == 0:
...
def plot_route(route, path='task3_route_mdp.png'):
    import matplotlib; matplotlib.use('Agg'); import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(10, 8))
    ra = np.array(route)
    ax.plot(ra[:, 0], ra[:, 1], '-o', lw=1.5, ms=3, color='steelblue')
    ax.plot(p.B[0], p.B[1], 'go', ms=12, label='B'); ax.plot(p.E[0], p.E[1], 'ro', ms=12, label='E')
    for i, s in enumerate(p.S):
    ...
    ax.set_xlabel('X'); ax.set_ylabel('Y'); ax.set_title('Task3 MDP+ScenarioTree Route')
...
def save_results(result, mc, path='task3_result_mdp.xlsx'):
    import pandas as pd
    if not result['feasible']: print('No feasible solution'); return

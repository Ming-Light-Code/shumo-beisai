...
% =========================================================================
%  solve_q3_online_opt.m 鈥?浠诲姟3 浼樺寲鐗堝湪绾块殢鏈哄喅绛栨ā鍨?(v2.0 淇鐗?
%  淇: 閲嶈鍒掑懆鏈?姣?澶?| 鐢熷瓨闃堝€?.5脳 | get_supply_needs浼犲叆wp_idx
%  鏀硅繘: P0宸紓鍖栬搴?+ P1棰勯槻鎬ц秴璐?+ P2鐢熷瓨浼樺厛 + P4浠ｇ爜閲嶆瀯
% =========================================================================
...
fprintf('========================================\n');
fprintf('  浠诲姟3 浼樺寲鐗堝湪绾块殢鏈哄喅绛? v2.0\n');
fprintf('========================================\n');
fprintf('P0: O脳%.2f H脳%.2f F脳%.2f | P1: 棰勯槻鎬ц秴璐?0%% | P2: 鐢熷瓨浼樺厛(1.5脳)\n', ...
    cons_exp.SAFETY_O, cons_exp.SAFETY_H, cons_exp.SAFETY_F);
fprintf('閲嶈鍒?: 澶╂皵鍙樺寲 + 姣?澶╁己鍒?\n');
fprintf('--- 閫愭棩鍐崇瓥 ---\n');
...
while state.day < cfg.MAX_DAYS && state.pt ~= 2
    ...
    % ---- P2: 鐢熷瓨浼樺厛妫€鏌?(淇: 浣跨敤1.5脳) ----
    dist_S1 = cfg.dist(state.pt, 6); dist_S2 = cfg.dist(state.pt, 7);
    nsd = min(dist_S1, dist_S2);
    % 浣跨敤鏈熸湜娑堣€?鍔犲畨鍏ㄨ搴?鐨?.5鍊?鍒版渶杩戣ˉ缁欑偣闇€姹?    survival_mode = (state.pt ~= 6) && (state.pt ~= 7) && ...

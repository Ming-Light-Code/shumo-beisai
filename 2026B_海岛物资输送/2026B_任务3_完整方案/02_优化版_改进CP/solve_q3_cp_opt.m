...
% solve_q3_cp_opt.m — 任务3 优化版离线CP求解器 (v2.0)
% 改进: P0差异化裕度(O×1.25,H/F×1.05) + P1预防性超购 + P4代码重构

cfg = cp_engine_opt('task3_config');
cons = cp_engine_opt('get_cons', 'expected', cfg);

fprintf('========================================\n');
fprintf('  任务3 优化版离线CP求解器 v2.0\n');
fprintf('========================================\n');
...
wa = []; ww = [];
for i = 2:length(best_path)
    if best_path(i) >= 3 && best_path(i) <= 5, wa(end+1) = i; ww(end+1) = best_path(i) - 2; end
end
[~, finalZ, finalM] = cp_engine_opt('simulate', best_path, m, cfg.dist, ...
    wa, ww, tt, best_works, best_parks, cons, cfg, init_s);

function solve_cg()
% solve_cg.m - Column Generation for Task 1 (FIXED + IDLE strategy)
% Fixes: supply-day resource consumption, complete daily output.
% Enhancement: idle-between-work strategy to bypass max_consec limit.
% Idle costs: O=1, H=1, F=1 per day, no Z gain. Minimum 1 idle day between blocks.
...
fprintf('========================================\n');
fprintf('  Column Generation for Task 1 (FINAL)\n');
fprintf('========================================\n\n');
fprintf('Features: idle-between-work blocks, correct resource accounting\n\n');

poolP={}; poolW={}; poolZ=[]; poolM=[];
pid0=[1,2]; [ok,Z0,M0,~]=gsim(pid0,0,dist(1,2),[],[],dist(1,2),[],all_xy,names,WM);
...
while true
    ...
    for sl=0:max_seq
        ...
        for si=1:ns
            ...
            nw=length(wa);
            if nw==0
                [ok,Z,M,dlog]=gsim(pid,m,tr,wa,ww,tt,[],all_xy,names,WM);
                if ok && Z>pi
                ...
                for ci=1:nc
                    ...
                    [ok,Z,M,dlog]=gsim(pid,m,tr,wa,ww,tt,wd,all_xy,names,WM);
                    if ok && Z>pi
...
[~,~,~,finalLog]=gsim(bP,m_opt,tr_opt,wa_opt,ww_opt,tt_opt,bW,all_xy,names,WM);
...
function [f,Zf,Mf,dailyLog] = gsim(pid,m,travel,wa,ww,tt,wdays,all_xy,names,WM)
T=tt;
if ~isempty(wdays)

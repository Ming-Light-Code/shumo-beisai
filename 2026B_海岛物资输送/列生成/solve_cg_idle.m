function solve_cg_idle()
% solve_cg_idle.m - Column Generation for Task 1 (with idle-between-work strategy)
% Allows: work block -> idle 1d -> work block at same location to bypass max_consec limit.
% Idle costs: O=1, H=1, F=1 per day, no Z gain.
...
fprintf('========================================\n');
fprintf('  Column Generation for Task 1 (IDLE)\n');
fprintf('========================================\n\n');
fprintf('Idle strategy: work blocks separated by 1 idle day\n');
fprintf('to reset consecutive counter at same location.\n\n');

poolP={}; poolW={}; poolZ=[]; poolM=[];
pid0=[1,2]; [ok,Z0,M0,~]=gsim_idle(pid0,0,dist(1,2),[],[],dist(1,2),[],all_xy,names,WM);
...
while true
    ...
    for sl=0:max_seq
        ...
        for si=1:ns
            ...
            nw=length(wa);
            if nw==0
                [ok,Z,M,dlog]=gsim_idle(pid,m,tr,wa,ww,tt,[],all_xy,names,WM);
                if ok && Z>pi
                ...
                avail=MAX_DAYS-tt; sz=zeros(1,nw); for jj=1:nw, sz(jj)=max(WM(ww(jj))+1, min(WM(ww(jj))*3, avail)+1); end; nc=prod(sz);
                for ci=1:nc
                    if found, break; end
                    ...
                    [ok,Z,M,dlog]=gsim_idle(pid,m,tr,wa,ww,tt,wd,all_xy,names,WM);
                    if ok && Z>pi
...
[~,~,~,finalLog]=gsim_idle(bP,m_opt,tr_opt,wa_opt,ww_opt,tt_opt,bW,all_xy,names,WM);
...
function [f,Zf,Mf,dailyLog] = gsim_idle(pid,m,travel,wa,ww,tt,wdays,all_xy,names,WM)
T=tt;
if ~isempty(wdays)

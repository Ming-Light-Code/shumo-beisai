...
class SkeletonSimulator:
        ...
        while state.day <= MAX_DAYS:
            ...
            if state.pos == target_pos:
                ...
                if node_name in self.work_gains:
                    ...
                    max_c = self.work_max[node_name]
                    # 计算时间紧迫度
                    # Fix: use actual remaining skeleton distance
                    rem_dist = 0
                    for j in range(seg_idx, len(skeleton)-1):
                        rem_dist += manhattan(self.node_pos[skeleton[j]], self.node_pos[skeleton[j+1]])
                    rem_dist += manhattan(state.pos, self.node_pos[skeleton[seg_idx]])
                    remaining = MAX_DAYS - state.day
                    urgency = remaining / max(1, rem_dist)

                    can_work = (weather == 0 and state.c < max_c)
                    storm_work = (weather == 1 and state.c < max_c and urgency < 1.5)

                    if can_work or storm_work:
                        action = 'work'
                        ...
                        state = ns
                    elif state.c < max_c:  # Fix: allow c=0 on first storm day
                        # 雷暴不紧急且还有配额: idle但留在工作点等待转晴
                        action = 'idle'
                        ns = self._apply_action(state, action, weather)
                        if ns is None: return None
                        # 保持c不变, 留在工作点继续等待
                    ...
                    ns_buy = self._apply_buy(state, buy_O, buy_H, buy_F)
                    if ns_buy is None: return None
    ...
    def _compute_buy(self, state, skeleton, seg_idx):
        ...
        改进: 用方差驱动的自适应 z-score 替代固定 1.2x 安全系数。
        - 预算充裕时 (z≈2.0): 多买, 确保雷暴天气下也不缺资源
        - 预算紧张时 (z≈0.5): 少买, 优先保留资金 (M)
        - 方差大的资源 (如燃油O, 正常2/雷暴8) → 需要更大缓冲
        """
        need_O, need_H, need_F = 0, 0, 0
        var_O, var_H, var_F = 0.0, 0.0, 0.0
        ...
        for j in range(seg_idx, next_supply_idx):
            ...
            # 方差: 每日消耗 i.i.d., Var(total) = d * Var(daily)
            e2_o = 0.8*(2**2)+0.2*(8**2); var_O += (e2_o-EXP_MOVE[0]**2)*d
            e2_h = 0.8*(3**2)+0.2*(4**2); var_H += (e2_h-EXP_MOVE[1]**2)*d
            e2_f = 0.8*(2**2)+0.2*(3**2); var_F += (e2_f-EXP_MOVE[2]**2)*d

            if n2 in self.work_gains:
                max_w = self.work_max[n2]
                exp_w = P_NORMAL * max_w
                need_O += EXP_WORK[0]*exp_w; need_H += EXP_WORK[1]*exp_w; need_F += EXP_WORK[2]*exp_w
                w_var = max_w*P_NORMAL*P_STORM
                var_O += w_var*(CONSUME_WORK[0][0]-CONSUME_WORK[1][0])**2
                var_H += w_var*(CONSUME_WORK[0][1]-CONSUME_WORK[1][1])**2
                var_F += w_var*(CONSUME_WORK[0][2]-CONSUME_WORK[1][2])**2

        # 自适应 z-score
        est_cost = need_O*PRICES[0]+need_H*PRICES[1]+need_F*PRICES[2]
        cur_val = state.O*PRICES[0]+state.H*PRICES[1]+state.F*PRICES[2]
        funds_needed = max(0, est_cost-cur_val)
        slack = (state.M-funds_needed)/max(1,funds_needed) if funds_needed>0 else 2.0
        z = 0.5 + 1.5*min(1.0,max(0,slack))  # Fix: z_min raised 0.3->0.5

        need_O = int(np.ceil(need_O + z*np.sqrt(max(0,var_O))))
        need_H = int(np.ceil(need_H + z*np.sqrt(max(0,var_H))))
        need_F = int(np.ceil(need_F + z*np.sqrt(max(0,var_F))))
...
class MDPSolver:
    ...
    def solve(self, n_simulations=500, top_k=5, refine=True):
        ...
        skeletons = self.enumerator.enumerate(max_intermediate=10, max_skeletons=10000)
        ...
        results.sort(key=lambda r: (r['avg_Z']*r['success_rate'], r['avg_M']), reverse=True)
        ...
        for _ in range(n_simulations):
            ...
            while s.day <= MAX_DAYS:
                ...
                if s.pos == tp:
                    ...
                    w=self.simulator.sample_weather()
                    dx=np.sign(tp[0]-s.x) or 0; dy=0
                    if dx==0: dy=np.sign(tp[1]-s.y) or 0
...
if __name__ == '__main__':
    ...
    best = solver.solve(n_simulations=500, top_k=5, refine=False)

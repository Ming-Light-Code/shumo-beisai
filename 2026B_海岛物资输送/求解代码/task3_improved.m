...
function tf = is_emergency(p, s, target)
    ...
    if s.O < wc_O + margin || s.H < wc_H + margin || s.F < wc_F + margin
        tf = true; return;
    end
...
function [feasible, seq, est_Z] = build_feasible_seq(p, D, s, wp_seq_names, completed)
    ...
    for wi = 1:length(wp_seq_names)
        ...
        if need_supply
            ...
            cur_day_tmp = s.day + d_to_sp + 1;

            exp_O_sp = d_to_sp * (p.P_NORM * p.CMOVE(1,1) + p.P_STORM * p.CMOVE(2,1));
            exp_H_sp = d_to_sp * (p.P_NORM * p.CMOVE(1,2) + p.P_STORM * p.CMOVE(2,2));
            exp_F_sp = d_to_sp * (p.P_NORM * p.CMOVE(1,3) + p.P_STORM * p.CMOVE(2,3));
            proj_O = s.O - exp_O_sp;
            proj_H = s.H - exp_H_sp;
            proj_F = s.F - exp_F_sp;
            ...
            need_O = rem_dist * (p.P_NORM * p.CMOVE(1,1) + p.P_STORM * p.CMOVE(2,1)) ...
                + p.WP_MAX(idx2) * (p.P_NORM * p.CWORK(1,1) + p.P_STORM * p.CWORK(2,1));
            need_H = rem_dist * (p.P_NORM * p.CMOVE(1,2) + p.P_STORM * p.CMOVE(2,2)) ...
                + p.WP_MAX(idx2) * (p.P_NORM * p.CWORK(1,2) + p.P_STORM * p.CWORK(2,2));
            need_F = rem_dist * (p.P_NORM * p.CMOVE(1,3) + p.P_STORM * p.CMOVE(2,3)) ...
                + p.WP_MAX(idx2) * (p.P_NORM * p.CWORK(1,3) + p.P_STORM * p.CWORK(2,3));
            need_O = need_O * p.SAFETY_FACTOR;
            need_H = need_H * p.SAFETY_FACTOR;
            need_F = need_F * p.SAFETY_FACTOR;

            buy_O = max(0, ceil(need_O - proj_O));
            buy_H = max(0, ceil(need_H - proj_H));
            buy_F = max(0, ceil(need_F - proj_F));
...
function task3_improved(seed, n_sim)

...
nodes=unique([initial.position;p.B;p.S;p.W;p.E], ...
...
% 构建宏观移动弧：只允许移动到功能点或已承诺目标
from=[];to=[];duration=[];
functionalOrCommitted=ismember(nodes,[p.B;p.S;p.W;p.E],'rows');
for i=1:nNode
...
for h=1:H
    ...
    lb(stateIdx(1:3,h+1))=max(lb(stateIdx(1:3,h+1)),reserve(:));
end
...
for h=1:H
    ...
    capCols=[stateIdx(1:3,h);allBuy];
    capVals=[ones(3,1);ones(3*nS,1)];
    addLe(capCols,capVals,p.capacity);

    moneyCost=repmat(p.price(:),nS,1);
    addLe([allBuy;stateIdx(4,h)],[moneyCost;-1],0);

    for r=1:3
        purchases=reshape(buyIdx(r,:,h),[],1);
        cols=[stateIdx(r,h+1);stateIdx(r,h);purchases];
        vals=[1;-1;-ones(nS,1)];
        for a=1:nArc
            d=duration(a);cost=d*p.expectedMove(r);
            if h==1,cost=weatherUse(todayWeather,p,'move',r)+(d-1)*p.expectedMove(r);end
    ...
    addEq([stateIdx(4,h+1);stateIdx(4,h);allBuy], ...
        [1;-1;moneyCost],0);
end

...
# ===== B v4 MC verification =====
B_MC_V4 = r"""function solve_q3_mdp_mc_v4(N)
...
write_file(DIR_B, 'solve_q3_mdp_mc_v4.m', B_MC_V4)

# ===== C v4 MC verification =====
C_MC_V4 = r"""function solve_q3_rso_mc_v4(N)
% SOLVE_Q3_RSO_MC_V4  Task3 RSO v4 MC Verification
...
fprintf('========================================\n');
fprintf('  Task3 RSO v4 MC (N=%d)\n',N);
fprintf('========================================\n');
fprintf('H=%dd K=%d(%dT) | Full-dist | Supply-route\n\n',rcfg.H,rcfg.K_total,rcfg.K_thunder);
...
for sim=1:N
    ws=cp_engine_v2('weather',90,0.8);
    [Zf,Mf,arr,days,reason]=run_rso_silent_v4(ws,rcfg);
    Zr(sim)=Zf; Mr(sim)=Mf; ok(sim)=arr; dy(sim)=days; fr{sim}=reason;
    if mod(sim,ri)==0
        fprintf(' %d/%d (%.0f%%) | %.1fs | Success: %.1f%%\n',...
            sim,N,100*sim/N,toc,100*sum(ok(1:sim))/sim);
...
function [Zf,Mf,arr,days,reason]=run_rso_silent_v4(wseq,rcfg)
    cfg=cp_engine_v2('config');
    cN=cp_engine_v2('cons','normal'); cT=cp_engine_v2('cons','thunder');
    ce=cp_engine_v2('cons','expected');
...
write_file(DIR_C, 'solve_q3_rso_mc_v4.m', C_MC_V4)
print('MC verification scripts written')

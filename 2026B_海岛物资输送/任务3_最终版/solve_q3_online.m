...
% =========================================================================
%  solve_q3_online.m — 任务3 在线随机天气决策模型 (最终版)
%  滚动时域CP重规划 | 三资源生存检查 | 场景CP | 预算自适应补给
%  成功率: 60% (MC N=50)
% =========================================================================

cfg = cp_engine('config');
cons_exp = cp_engine('cons', 'expected');
cons_N   = cp_engine('cons', 'normal');
cons_T   = cp_engine('cons', 'thunder');

if nargin < 1 || isempty(weather_seq)
    rng('shuffle'); weather_seq = cp_engine('weather', 90, 0.8);
end
...
while st.day<cfg.MAX_DAYS && st.pt~=2
    ...
    surv=(st.pt~=6)&&(st.pt~=7)&&((st.O<1.0*nsd*cons_exp.MO)||(st.H<1.0*nsd*cons_exp.MH)||(st.F<1.0*nsd*cons_exp.MF));

    % 重规划触发
    an=(sil==0&&pil==0); naw=~(st.pt>=3&&st.pt<=5);
    wc=(st.day>1&&w~=weather_seq(max(1,st.day-1)));
    per=(mod(st.day,3)==1&&st.day>1);
    nr=isempty(pp)||((wc||per)&&an&&naw)||(surv&&an&&naw&&~isempty(pp)&&any(pp(2:end-1)>=3&pp(2:end-1)<=5));

    if nr
        rc=rc+1; el=st.day-1;
        is=struct('O',st.O,'H',st.H,'F',st.F,'M',st.M,'Z',st.Z);
        if surv&&st.pt~=6&&st.pt~=7
            sc=sc+1; [pp,pprk,pwrk,fok]=cp_engine('plan',st.pt,el,cons_N,cfg,true,is);
        else
            [pp,pprk,pwrk,fok]=cp_engine('plan_scenario',st.pt,el,cfg,false,is);
        end
        if ~fok
            [pp,pprk,pwrk,fok]=cp_engine('plan',st.pt,el,cons_exp,cfg,false,is);
        end
        if ~fok
            [pp,pprk,pwrk,fok]=cp_engine('plan',st.pt,el,cons_N,cfg,surv,is);
    ...
    if nw
        ...
        if st.consec<wm
            ...
            act='park(reset)'; st.O=st.O-ca.PO; st.H=st.H-ca.PH; st.F=st.F-ca.PF; st.consec=0; ik=true;
        end
    elseif pl<=length(pprk)&&pil<pprk(pl)
        act='park(at sea)'; st.O=st.O-ca.PO; st.H=st.H-ca.PH; st.F=st.F-ca.PF; st.consec=0; pil=pil+1; ik=true;
    elseif sil<cfg.dist(st.pt,np)
        ...
        if sil>=cfg.dist(st.pt,np)
            ...
            if st.pt==6||st.pt==7
                [nO,nH,nF]=cp_engine('supply_needs',pp,pprk,pwrk,pl,cons_exp,cfg);
    ...
    if ik||nr
        tg=''; if nr, tg=sprintf('[R#%d]',rc); end; if surv, tg=[tg '[S]']; end
...
fprintf('-----|---|---------------------|------------|----------------|---------\n');
fprintf('\n===== 结果 =====\n');
if st.pt==2, fprintf('Day %d 抵达E | Z=%d M=%.0f | R:%d S:%d\n',st.day,st.Z,round(st.M),rc,sc);
elseif st.day>=cfg.MAX_DAYS, fprintf('超时 | Z=%d M=%.0f\n',st.Z,round(st.M));
else, fprintf('失败(Day %d) | Z=%d M=%.0f\n',st.day,st.Z,round(st.M)); end
fprintf('Done.\n');
end

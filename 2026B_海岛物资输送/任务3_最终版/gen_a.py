...
DIRS = {
    "A": os.path.join(BASE, "优化A_框架内修复"),
    "B": os.path.join(BASE, "优化B_MDP方向"),
    "C": os.path.join(BASE, "优化C_滚动随机优化"),
}

for d in DIRS.values():
    os.makedirs(d, exist_ok=True)

# ==== Approach A: cp_engine_v2.m ====
CP_ENGINE_V2 = """function varargout = cp_engine_v2(action, varargin)
% CP_ENGINE_V2  Task3 CP Engine v2 (Weather-Adaptive)
% Improvements: thunder-safe supply, improved pruning, safe-mode path-to-E
...
switch action
    ...
    case 'supply_needs_safe', [varargout{1},varargout{2},varargout{3}] = get_supply_needs_safe(varargin{:});
    case 'weather',      varargout{1} = gen_weather(varargin{:});
    case 'path_to_E',    varargout{1} = plan_to_E(varargin{:});
    otherwise, error('cp_engine_v2: unknown action');
end
end
...
function [bp, bprk, bwrk, ok] = plan_from_state(cur_pt, elapsed, cons, cfg, survival, init_s)
    ...
    [bZ,bM,bp,bwrk,bprk]=cp_search([cur_pt],0,[],[],bZ,bM,bp,bwrk,bprk,cfg,cons,eff,init_s,init_s.Z);
    ok=(bZ>-inf);
end
...
function [bp, bprk, bwrk, ok] = plan_scenario(cur_pt, elapsed, cfg, survival, init_s)
    ...
    cl={get_cons('normal'),get_cons('expected'),get_cons('thunder')};
    bZ=-inf; bM=-inf; bp=[cur_pt,2]; bwrk=[]; bprk=[];
    for sc=1:3
...
function [bZ,bM,bP,bWD,bPS]=cp_search(path,tsf,wa,ww,bZ,bM,bP,bWD,bPS,cfg,cons,eff,init_s,accZ)
    lp=path(end);
    if lp~=2
        dE=cfg.dist(lp,2); rem=eff-tsf;
        if rem<dE, return; end
        if accZ+mwp(3,rem-dE)*28<=bZ&&bZ>-inf, return; end
    end
    dE=cfg.dist(lp,2);
    if tsf+dE<=eff
        ...
        if nw==0
            ...
            mx=zeros(1,nw);
            for j=1:nw, mx(j)=mwp(cfg.WM(ww(j)),rem_d); end
            sz=mx+1; nc=prod(sz);
            for ci=1:nc
                ...
                ts=0;
                for j=1:nw
                    if wd(j)>0, ts=ts+wd(j)+max(0,ceil(wd(j)/cfg.WM(ww(j)))-1); end
                end
    ...
    for ni=1:5
        ...
        [bZ,bM,bP,bWD,bPS]=cp_search(np2,nt,nwa,nww,bZ,bM,bP,bWD,bPS,cfg,cons,eff,init_s,accZ);
    end
end
...
function [ok,Zf,Mf]=simulate_path(pid,m,dist_all,wa,ww,tt,wdays,park_seg,cons,cfg,init_s)
    ...
    cO=zeros(1,Ta); cH=zeros(1,Ta); cF=zeros(1,Ta);
    zG=zeros(1,Ta); iS=false(1,Ta);
    day=0;
    for k=1:m+1
        ...
        if ~isempty(wa)
            ...
            if ~isempty(wk)&&~isempty(wdays)&&wk<=length(wdays)&&wdays(wk)>0
                ...
                while rv>0
                    ...
                    rv=rv-ch;
                    if rv>0, day=day+1; cO(day)=cons.PO; cH(day)=cons.PH; cF(day)=cons.PF; end
                end
            end
        end
    end
    ...
    for t=1:Ta
        ...
        if iS(t)
            ...
            if t~=lsi
                ...
                if sab>1e-6
                    bt=0.15*sab;
                    tot=cons.MO+cons.MH+cons.MF;
                    bO=bO+bt*(cons.MO/tot);
                    bH=bH+bt*(cons.MH/tot);
                    bF=bF+bt*(cons.MF/tot);
                end
            end
            if bO+bH+bF>sp+1e-6, ok=false; Mf=0; return; end
            if ns>Ta&&(O+bO<nO-1e-6||H+bH<nH-1e-6||F+bF<nF-1e-6)
                ok=false; Mf=0; return;
            end
    ...
    for k=plan_leg:length(plan_path)-2
        ...
        if plan_path(k+1)>=3&&plan_path(k+1)<=5
            ...
            if wc<=length(plan_works)
                ...
                wO=wO+wd*cons.WO+np*cons.PO;
                wH=wH+wd*cons.WH+np*cons.PH;
                wF=wF+wd*cons.WF+np*cons.PF;
            end
        end
    end
    nO=rt*cons.MO+rp*cons.PO+wO;
    nH=rt*cons.MH+rp*cons.PH+wH;
    nF=rt*cons.MF+rp*cons.PF+wF;
end

function [nO,nH,nF]=get_supply_needs_safe(plan_path,plan_parks,plan_works,plan_leg,cons,cfg)
    ct=get_cons('thunder');
    [nO_e,nH_e,nF_e]=get_supply_needs(plan_path,plan_parks,plan_works,plan_leg,cons,cfg);
    [nO_t,nH_t,nF_t]=get_supply_needs(plan_path,plan_parks,plan_works,plan_leg,ct,cfg);
    nO=0.70*nO_e + 0.30*nO_t;
    nH=0.70*nH_e + 0.30*nH_t;
    nF=0.70*nF_e + 0.30*nF_t;
end

function pp = plan_to_E(cur_pt, elapsed, cfg)
    if nargin<3, cfg=get_config(); end
    dE=cfg.dist(cur_pt,2);
    if elapsed+dE<=cfg.MAX_DAYS
        pp=[cur_pt, 2];
    else
        pp=[];
    end
...
function ws=gen_weather(n,pn)
    ws=repmat('N',1,n);
    for i=1:n, if rand()>pn, ws(i)='T'; end; end
end
"""

with open(os.path.join(DIRS["A"], "cp_engine_v2.m"), "w", encoding="utf-8") as f:
    f.write(CP_ENGINE_V2)

print("cp_engine_v2.m written successfully")
